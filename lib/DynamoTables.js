...
DynamoTables.prototype.shortTableName = function (tableName) {
    ...
    assert(this.rawTables[shortName], "The shortened name is not a valid table");
...
/*
 * Returns the name of the field in the object that should be used
 *   as index for certain table
 */
DynamoTables.prototype.getKeyFieldForTable = function (tableName) {
    ...
    // FIXME: Asuming one unique key
    return tableDesc.KeySchema[0].AttributeName;
};
...
DynamoTables.prototype.getKeyForItem = function (item, tableName) {
    // FIXME: Asuming one unique key (in getKeyField...)  and type string
    return item[this.getKeyFieldForTable(tableName)]["S"];
};
