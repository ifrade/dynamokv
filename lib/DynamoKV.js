...
function DynamoKV(tableNames, tableDefinitions) {
    assert(tableNames, "Table names object is mandatory to create a DynamoKV");
    assert(typeof tableNames === 'object',
           "Table names must be an object (old code using a prefix?");
    ...
    this.tables = new DynamoTables(tableNames, tableDefinitions);
}


function createDynamoKV(tableNames, tableDefinitions, callback) {
    var x = new DynamoKV(tableNames, tableDefinitions, callback);
    callback(null, x);
}
...
DynamoKV.prototype.putOnTable = function (tableName, key, value, overwrite, callback) {
    ...
    var otherFields = this.tables.getOtherFieldsForTable(tableName);
    for (var i = 0; i < otherFields.length; i++) {
        var field = otherFields[i];
        if (value.hasOwnProperty(field)) {
            operation.Item[field] = { "S" : value[field] };
        } else {
            return callback(new Error("Object for table" + tableName +
                                      "should have property" + otherFields[i]));
        }
    }
...
/* callback (err, [{ range: x, content: y},...] */
DynamoKV.prototype.listOnIndexKey = function (tableName, idxName, hashKey, callback) {
    var fullTableName = this.tables.fullTableName(tableName);
    var keyFields = this.tables.getKeyFieldForIndex(tableName, idxName);
...
DynamoKV.prototype.emptyTables = function (callback) {
    ...
    async.each(this.tables.getFullTableNames(), function (tableName, cb) {
